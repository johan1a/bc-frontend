...
The `home-routes` handler in the `bc-frontend.routes.home` namespace
...
The `home-page` function will in turn call the `bc-frontend.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `bc-frontend.handler` namespace:
...
Request middleware functions are located under the `bc-frontend.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `bc-frontend.dev-middleware` namespace found in
